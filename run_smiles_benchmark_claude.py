#!/usr/bin/env python3
"""Run the benchmark against an Anthropic model with no tools.

Design goals:
- **No extended thinking.** It competes for output token budget and can truncate JSON.
- **No tool use.** We want to probe the model's raw chemical reasoning ability.
- **Generous max_tokens** to guarantee room for the full answer list without truncation.
- **Single-turn auditability.** We do not retry malformed or incomplete outputs; we
  record the failure so the benchmark can expose formatting and tokenizer issues.
- **Full response trace** (every content block with type + text/json dump) is written
  so we can audit whether a hidden tool was used.
- **No letter grade.** Just the numeric score / max / percent.

Requirements: ANTHROPIC_API_KEY, anthropic, RDKit.
"""
...
def _anthropic_text_response(response) -> str:
    parts: list[str] = []
...
def _dump_response_trace(response) -> list[dict]:
    ...
    for i, block in enumerate(response.content):
        ...
        # dump every field we can see
        for attr in ("text", "name", "id", "input", "thinking", "signature"):
            if hasattr(block, attr):
                try:
                    val = getattr(block, attr)
                    entry[attr] = val if isinstance(val, (str, int, float, bool, list, dict, type(None))) else repr(val)
                except Exception:
                    pass
        trace.append(entry)
    return trace
...
def _usage_dict(response) -> dict | None:
    ...
    ce = getattr(usage, "cache_creation_input_tokens", None)
    if ce is not None:
        out["cache_creation_input_tokens"] = ce
...
def main() -> None:
    ...
    final_text = json.dumps(final_payload if final_payload else {"answers": []}, indent=2)
    ...
    if secret_module is not None and hasattr(secret_module, "grade_submission"):
        trusted_grade = secret_module.grade_submission(final_payload if isinstance(final_payload, dict) else {"answers": []})
